...
#   bash scripts/demo_e2e_mac.sh --auto --no-type   # recording: SSH-runs the VM scripts too
#
set -euo pipefail
...
commands from docs/admin/RHEL_TWO_HOST.md (ping, compile-on-dev, canary, enforce, RPMs).
...
tlab_explain "rsync copies this laptop checkout onto rhel-dev (that tree is often not a git clone). Prod gets only the tiny talk-track folder — no git clone."
e2e_run "bash scripts/sync_rhel_dev.sh"
e2e_run "ssh ${E2E_SSH_USER}@${PROD_HOST} 'mkdir -p ~/e2e-demo/lib'"
...
tlab_print_section "Part 3 — Compile rules on rhel-dev, canary from the Mac"
tlab_explain "A Mac cannot compile SELinux. rhel-dev has selinux-policy-devel. We compile there, then copy myapp.pp back so Ansible on this laptop can ship it."
e2e_run "ssh ${E2E_SSH_USER}@${DEV_HOST} 'cd ~/selinux-pac && bash scripts/compile_and_validate.sh selinux'"
e2e_run "scp ${E2E_SSH_USER}@${DEV_HOST}:~/selinux-pac/selinux/myapp.pp selinux/myapp.pp"
...
tlab_print_section "Part 6 — Build RPMs on rhel-dev, copy to prod"
tlab_explain "Production must not git clone this repo. We ship installer files (RPMs), like a .pkg on a Mac. rpmbuild runs on rhel-dev; this laptop just collects dist/*.rpm."
...
Press Enter here when rpm -q selinux-policy-ops myapp-selinux succeeds." \
    "ssh ${E2E_SSH_USER}@${PROD_HOST} 'bash ~/e2e-demo/demo_e2e_rhel_prod.sh $(e2e_auto_flags)'"
