...
> Full principles and anti-patterns: [`docs/SELINUX_BEST_PRACTICES.md`](../SELINUX_BEST_PRACTICES.md) (§8 review checklist).
...
| **Soak Period (7–14 days)** | ⬜ Pending | `check_soak_ready.sh` + daily `monitor_avc.sh` on staging/prod canary |
...
**Production enforce (manual):** After staging sign-off and soak, admin runs **SELinux Policy Deploy** workflow → `enforce` on `production` (GitHub Environment approval required). See [`docs/PRODUCTION_READINESS.md`](../PRODUCTION_READINESS.md).
